...
class PurchaseOrder(Base):
    ...
    po_id = Column(Integer, primary_key=True, autoincrement=True, comment='รหัส PO (Auto Increment)')
    po_poid = Column(Integer, comment='รหัสอ้างอิง PO')
    ...
    po_system = Column(String(10), comment='ระบบ')
    po_department = Column(String(10), comment='แผนก')
    po_memid = Column(String(10), comment='รหัสพนักงาน')
    po_prname = Column(String(100), comment='ชื่อ PR')
    po_trem = Column(String(100), comment='เงื่อนไขการชำระเงิน')
    po_contact = Column(String(300), server_default='', comment='ผู้ติดต่อ')
    po_prno = Column(String(30), comment='เลขที่ PR')
    po_contactno = Column(String(100), comment='เบอร์ติดต่อ')
    po_quono = Column(String(30), comment='เลขที่ใบเสนอราคา')
    ...
    po_status = Column(String(30), server_default='enable', comment='สถานะ')
    po_venderid = Column(Integer, comment='รหัสผู้ขาย')
    po_vender = Column(String(100), comment='ชื่อผู้ขาย')
    po_open = Column(String(45), server_default='no', comment='สถานะการเปิด')
    po_approve = Column(String(45), server_default='wait', comment='สถานะการอนุมัติ')
    po_disapprove = Column(Text, comment='เหตุผลที่ไม่อนุมัติ')
    po_qty = Column(Integer, comment='จำนวน')
    ic_status = Column(String(5), server_default='wait', comment='สถานะ IC')
    apv_open = Column(String(10), server_default='no', comment='สถานะการเปิดอนุมัติ')
    compcode = Column(String(45), comment='รหัสบริษัท')
    useradd = Column(String(45), comment='ผู้เพิ่มข้อมูล')
    usercreate = Column(DateTime , default=datetime.now(), comment='วันที่สร้าง')
    useredit = Column(String(45), comment='ผู้แก้ไขข้อมูล')
    editdate = Column(DateTime, comment='วันที่แก้ไข')
    userdelete = Column(String(45), comment='ผู้ลบข้อมูล')
    deletedate = Column(DateTime, comment='วันที่ลบ')
    po_vat = Column(Numeric(10,4), comment='มูลค่า VAT')
    ...
    reject_remark = Column(String(300), comment='เหตุผลที่ปฏิเสธ')
    reject_user = Column(String(300), comment='ผู้ปฏิเสธ')
    dis_remark = Column(String(255), comment='หมายเหตุส่วนลด')
    dis_user = Column(String(255), comment='ผู้ให้ส่วนลด')
    downper = Column(Numeric(12,2), comment='เปอร์เซ็นต์เงินดาวน์')
    down = Column(Numeric(12,2), comment='จำนวนเงินดาวน์')
    sumdown = Column(Numeric(12,2), comment='รวมเงินดาวน์')
    retentionper = Column(Numeric(12,2), comment='เปอร์เซ็นต์เงินประกัน')
    retention = Column(Numeric(12,2), comment='จำนวนเงินประกัน')
    sumretention = Column(Numeric(12,2), comment='รวมเงินประกัน')
    status_down = Column(String(255), comment='สถานะเงินดาวน์')
    status_retention = Column(String(255), comment='สถานะเงินประกัน')
    wo_ref = Column(String(255), comment='อ้างอิง WO')
    poi_deduct_status = Column(String(50), comment='สถานะการหัก')
    is_decrement = Column(String(10), server_default='no', comment='สถานะการลด')
    contact_store = Column(String(40), comment='ผู้ติดต่อร้านค้า')
    teamother = Column(String(255), comment='ทีมอื่นๆ')
